...
public class WoodcutterScreen extends HandledScreen<WoodcutterScreenHandler> {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, identifier, i + 119, j + 15 + k, 12, 15);
        ...
        if (this.canCraft) {
            ...
            for (int l = this.scrollOffset; l < k && l < this.handler.getAvailableRecipeCount(); l++) {
                ...
                if (x >= n && x < n + 16 && y >= o && y < o + 18) {
                    ItemStack tipStack = list.get(l).value().result().copy();
                    // Feature-gate & world null safety
                    if (this.client.world == null || !tipStack.isItemEnabled(this.client.world.getEnabledFeatures())) {
                        tipStack = ItemStack.EMPTY;
                    }
                    context.drawItemTooltip(this.textRenderer, tipStack, x, y);
                }
            }
        }
    }
    ...
    private void renderRecipeIcons(DrawContext context, int x, int y, int scrollOffset) {
        ...
        for (int i = this.scrollOffset; i < scrollOffset && i < this.handler.getAvailableRecipeCount(); i++) {
            ...
            ItemStack out = list.get(i).value().result().copy();
            // Feature-gate & world null safety
            if (this.client.world == null || !out.isItemEnabled(this.client.world.getEnabledFeatures())) {
                out = ItemStack.EMPTY;
            }
            context.drawItem(out, k, m);
        }
    }
            ...
            for (int l = this.scrollOffset; l < k; l++) {
                ...
                if (d >= 0.0 && e >= 0.0 && d < 16.0 && e < 18.0 && this.handler.onButtonClick(this.client.player, l)) {
                    MinecraftClient.getInstance().getSoundManager()
                            .play(PositionedSoundInstance.master(SoundEvents.BLOCK_WOODEN_BUTTON_CLICK_ON, 1.0F));
